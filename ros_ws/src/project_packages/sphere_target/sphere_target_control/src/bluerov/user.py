...
class Code(object):
    ...
    def __init__(self):
        ...
        # Thrusters Input
        #self.pub.subscribe_topic('/sphere_target/thrusters/0/input', FloatStamped)
        #self.pub.subscribe_topic('/sphere_target/thrusters/1/input', FloatStamped)
        #self.pub.subscribe_topic('/sphere_target/thrusters/2/input', FloatStamped)
        #self.pub.subscribe_topic('/sphere_target/thrusters/3/input', FloatStamped)
        #self.pub.subscribe_topic('/sphere_target/thrusters/4/input', FloatStamped)
        #self.pub.subscribe_topic('/sphere_target/thrusters/5/input', FloatStamped)
    ...
    def pwm_to_thrust(pwm):
        ...
        The equation come from:
            ...
            In our case we are using T100 Thrusters on sphere_target
        Args:
            pwm (int): pwm value
    ...
    def run(self):
		...
		# override.append(1300)	override len is 8!!!
            ...
            #try:
                ...
                #self.pub.set_data('/sphere_target/thrusters/0/input', _input)

                #_input.data = self.pwm_to_thrust(rc[1])
                #self.pub.set_data('/sphere_target/thrusters/1/input', _input)

                #_input.data = self.pwm_to_thrust(rc[2])
                #self.pub.set_data('/sphere_target/thrusters/2/input', _input)

                #_input.data = self.pwm_to_thrust(rc[3])
                #self.pub.set_data('/sphere_target/thrusters/3/input', _input)

                #_input.data = self.pwm_to_thrust(rc[4])
                #self.pub.set_data('/sphere_target/thrusters/4/input', _input)

                #_input.data = self.pwm_to_thrust(rc[5])
                #self.pub.set_data('/sphere_target/thrusters/5/input', _input)
